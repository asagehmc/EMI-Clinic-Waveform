...
def ecg_staging(path, dataset):
    ...
    # load SleepECG weighted "wrn-gru-mesa-weighted" classifier (requires tensorflow)
...
def save_predictions(pred, file_path):
    """
    Saves the weighted predicted sleep stage annotations to a CSV file in a different folder.
    """
    # Convert the weighted predictions to a DataFrame
...
if __name__ == "__main__":
    ...
    for edf in edfs:
        if edf[-4:] == ".edf":
            ...
            # Save the weighted predictions to a CSV file
            output_folder = "output_predictions/"
            os.makedirs(output_folder, exist_ok=True)
            save_predictions(pred, f"{output_folder}{edf[:-4]}_wgt_predictions.csv")
